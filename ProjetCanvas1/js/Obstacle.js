...
export class MovingObstacle extends Obstacle {
    ...
    draw(ctx) {
        if (this.image.complete && this.image.naturalWidth !== 0) {
            ...
            if (this.h > this.w) {
                // Vertical : on tourne de 90 degrés
...
export class PipeObstacle extends Obstacle {
    constructor(x, y, w, h, angle = 0, topCap = true, bottomCap = true, flipped = false) {
        super(x, y, w, h, "gray");
        this.angle = angle;
        this.topCap = topCap;
    ...
    draw(ctx) {
        if (!this.bodyImg.complete || !this.capImg.complete || this.bodyImg.naturalWidth === 0) {
            super.draw(ctx);
            return;
        }
        ...
        // Gestion conditionnelle des bouchons (caps)
        if (this.topCap) {
        ...
        // Le corps du tuyau remplit l'espace restant
        if (currentH > 0) {
